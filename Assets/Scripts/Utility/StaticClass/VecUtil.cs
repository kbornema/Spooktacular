...
{
    public static Vector2 AsPos2(this Transform t)
    {
        return t.position;
    }
    ...
    {
        return RoundToNearestVector2f(input, new Vector2(division, division));
    }

    public static Vector2 RoundToNearestVector2f(Vector2 input, Vector2 division)
    {
        return new Vector2( (float)(Math.Round(input.x * division.x, MidpointRounding.AwayFromZero)) / division.x,
                            (float)(Math.Round(input.y * division.y, MidpointRounding.AwayFromZero)) / division.y);
    }
