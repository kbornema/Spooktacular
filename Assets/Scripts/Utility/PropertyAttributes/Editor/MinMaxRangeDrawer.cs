...
public override void OnGUI( Rect position, SerializedProperty property, GUIContent label )
{
    ...
    else
    {
        ...
        var minValue = property.FindPropertyRelative( "_min" );
        var maxValue = property.FindPropertyRelative( "_max" );
        var newMin = minValue.floatValue;
        var newMax = maxValue.floatValue;

        var xDivision = position.width * 0.33f;
        var yDivision = position.height;
        ...
        //EditorGUI.LabelField(new Rect(position.x + position.width - 20.0f, position.y + yDivision, position.width, yDivision), range.maxLimit.ToString( "0.##" ));
        ...
        EditorGUI.LabelField(new Rect( position.x + xDivision - 6.0f, position.y, xDivision, yDivision ), "Min: " );

        newMin = Mathf.Clamp(EditorGUI.FloatField( new Rect( position.x + xDivision + 32.0f, position.y, xDivision - 36.0f, yDivision ), newMin ), range.minLimit, newMax );
        EditorGUI.LabelField(new Rect( position.x + xDivision * 2f, position.y, xDivision, yDivision ), "Max: " );
        newMax = Mathf.Clamp(EditorGUI.FloatField( new Rect( position.x + xDivision * 2.0f + 30.0f, position.y, xDivision - 30.0f, yDivision ), newMax ), newMin, range.maxLimit );
